...
class Ackermann() :
    ...
    def __init__(self, robot_state, robot_parameters,
        ...
        self._paramters = robot_parameters
    ...
    def step(self, steer_input, vel_input, Ts=None, sideslip_angles=[0.0, 0.0]) :
        ...
        yawNew, yawRateNew = self.calc_yawModel(state)
    ...
    def calc_steerModel(self, state, steer_input):
        Ts = self._paramters.sampling_time()
        ...
        if self._steer_model is None:
            ...
            steerRateNew = 1 / T_steer * (steer_input - state["steer"])
            steerNew = state["steer"] + Ts * steerRateNew

        elif self._steer_model == "2st-order-lag":
            T1_steer, T2_steer = self._paramters.timeConstants_steer_2ndOrderLag()
            steerRateNew = state["steerRate"] + 1 / T2_steer * (steer_input - T1_steer * state["steerRate"] - state["steer"])
            steerNew = state["steer"] + Ts * state["steerRate"] # just a integration
